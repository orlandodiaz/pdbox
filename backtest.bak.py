...
class Backtest(object):
    def __init__(self, strategy, stock_list):
        ...
        self._is_done = False
        self.df = 0
        self.stocks_list = stock_list
        self.buy_start = '2017-02-01'
        self.buy_end = '2017-11-08'
    ...
    def _individual_backtest(self, stock, buy_start, buy_end):
        ...
        if self.extended_hours:
            self.df = self.df.ix[self.df.index.indexer_between_time(datetime.time(9,30), datetime.time(16))]

        # Buying range: Date must be the index
        self.df = self.df.loc['{buy_start} 9:30'.format(buy_start=self.buy_start):'{buy_end} 16:00'
            .format(buy_end=self.buy_end)]

        self.df = self.df.reset_index()
        self.df = self.df.rename({'index': 'date'}, axis='columns')

        index_array = self.strategy.get_buy_coordinates(self.df)

        #self.strategy.check_for_conditions(self, self.df, ticker, index_array)
        ...
        for index in index_array:
            ...
            buy_price = self.df.iloc[index]['close']
            buy_date = self.df.iloc[index]['date']

            init = int(index) # Initial value
            volume = self.df.iloc[index]['volume']

            sell_date, sell_price = self.strategy.sell_algorithm(init, ticker, self.df)
    ...
    def save_results(self):
        ...
        self.long_winners = len(self.trade_log[(self.trade_log['pct_change'] > 0)])
        self.long_losers = len(self.trade_log[(self.trade_log['pct_change'] < 0)])

        self.winning_trades = self.long_winners
        self.losing_trades = self.long_losers

        self.short_winners = len(self.trade_log[(self.trade_log['pct_change'] < 0)])
        self.short_losers = len(self.trade_log[(self.trade_log['pct_change'] > 0)])

        if self.short_winners and self.short_losers != 0:
            self.winning_percent = (float(self.short_winners) / (self.short_losers + self.short_winners) * 100)
        else:
            self.winning_percent = 0
    ...
    def print_results(self):
       ...
       # print 'Winning percent', self.long_winners / (self.long_losers + self.long_winners)
        ...
        print 'pct_change avg: ', self.trade_log['pct_change'].mean()
