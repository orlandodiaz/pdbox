...
class ADXStrat(Strategy):
    ...
    def get_buy_coordinates(self, df):
        buy_locations = df.index[
            (df['chg_since_open'] > 5) & (df['adx'] < 30) &
            # (df['avg_vol_14d'] > 80000) &
            # (df['close'] < df['ema13'])
            (df['close'] > 1) & (df['close'].shift(1) < df['ema13'].shift(1)) &
            (df['open'] - df['close'] < 0)
            &  # If we have a positive candlestick
            ...
            (df['volume'] > 120000)
            &  #50,000 is not a lot of volume to start with. Higher than 100k ensures this is not a false alarm
            ...
            ((df['volume'] / df['7dayMaxVol']) > 1.25
            )  # Not really a 7day high if we don't have enough data. Common issue with alphavantage which only returns the previous 15 days. Previously sued 2.5
    ...
    def check_for_conditions(self, df, ticker, buy_locations):
        ...
        if len(buy_locations) == 0:
            ...
            if (df['volume'] > 130000).any() is False:
                print '{ticker}\t Volume condition failed. Volume not greater than 130,000'.format(
                    ticker=ticker)
            elif (df['high'] > df['4mo_high']).any() is False:
                print '{ticker} \t Current high is not greater than the 4 month high'.format(
                    ticker=ticker)
        ...
        for i in range(init, len(df) + 1):
            ...
            if (df.iloc[[i + 1]]['close'] > df.iloc[[i + 1
                ...
                print "%s backtester2: \t %s \t SELL @ %.3f\t LOC: %d . Sold solely on close < ema" \
                      % (datetime.now().strftime("%H:%M:%S"), ticker, sell_price, i+1)
                break
